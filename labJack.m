...
%> can then send digital I/O commands and strobed words. labJack ises a 12bit
...
classdef labJack < handle
	...
	%=======================================================================
		...
		%> @param count
		% ===================================================================
		function [in,retBytes] = rawRead(obj,bytein,count)
			if ~exist('bytein','var')
				bytein = zeros(10,1);
			end
			...
			[in,retHandle, retBytes] =  calllib('liblabjackusb', 'LJUSB_ReadTO', obj.handle, bytein, count, obj.timeOut);
